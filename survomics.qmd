...
The function `risksetROC::risksetAUC()` can use `cv.glmnet`'s output linear predictors (`lp`) as markers for estimating time-dependent AUC, i.e. areas under ROC curve at given evaluation time points.
:::
...
# calculate the time-dependent AUC from the training and validation data sets
AUC_train <- risksetROC::risksetAUC(
  Stime = y_train[, "time"],
  status = y_train[, "status"],
  marker = pred_lp_train,
  tmax = max(y_train[, "time"])
)
AUC_validate <- risksetROC::risksetAUC(
  Stime = y_validate[, "time"],
  status = y_validate[, "status"],
  marker = pred_lp_validate,
  tmax = max(y_validate[, "time"])
)
# draw the time-dependent AUC
dat_AUC <- data.frame(
  tAUC = c(AUC_train$AUC, AUC_validate$AUC),
  times = c(utimes_train, utimes_validate),
  group = c(
    rep("AUC_train", length(AUC_train$AUC)),
    rep("AUC_validate", length(AUC_validate$AUC))
  )
)
...
(iAUC_train <- risksetROC::IntegrateAUC(AUC_train$AUC, AUC_train$utimes, AUC_train$St, tmax = 10))
```
```{.border}
[1] 0.6758971
```
...
(iAUC_validate <- risksetROC::IntegrateAUC(AUC_validate$AUC, AUC_validate$utimes, AUC_validate$St, tmax = 10))
```
```{.border}
[1] 0.6602153
```
...
The function `riskRegression::Score()` works with predicted event probabilities as input, i.e. $R(t|X)=1-S(t|X)$ where $S(t|X)$ is the predicted survival chance for subject $X$ at time $t$. 
...
It returns both time-dependent Brier score and IBS at given evaluation time points as follows.
