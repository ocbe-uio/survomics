...
```
<br>
This is an [R Markdown](http://rmarkdown.rstudio.com) supplement for the article **_Tutorial on survival modelling with omics data_**.
...
```{r}
meta = colData(dat)[, c("project_id", "submitter_id", "age_at_diagnosis",  "ethnicity", "gender", "days_to_death", "days_to_last_follow_up", "vital_status", "paper_BRCA_Subtype_PAM50")]
...
theme_set(theme_bw())
survminer::ggsurvplot(sfit, conf.int = TRUE, risk.table = TRUE, 
                      xlab = "Time since diagnosis (year)",legend = "none",
                      surv.median.line = "hv")
```
![_Kaplan-Meier curve for 1061 BRCA patients data from TCGA._](fig/TCGA_surv_km1.png){width=60%}

To compare the survival curves of two groups of patients, for example, age$<60$ vs age$\ge 60$, the `R` function `survival::survdiff()` can perform the log-rank test to compare two survival curves.
Alternatively, the `R` function `survival::survfit` with a formula including the age group as a covariate can return the (KM) survival probabilities for each groups. 
...
```{r}
clin$age2 = clin$age
clin$age2[clin$age2 < 60] = 0
clin$age2[clin$age2 >= 60] = 1
survival::survdiff(Surv(time, status) ~ age2, data = clin)

sfit2 = survfit(Surv(time, status) ~ age2, data = clin)
ggsurv = ggsurvplot(sfit2, conf.int = TRUE, risk.table = TRUE, 
           xlab = "Time since diagnosis (year)", legend = c(.8,.9),
           legend.labs = c("<60", ">=60"), legend.title = "Age",  
           risk.table.y.text.col = TRUE, risk.table.y.text = FALSE)
ggsurv$plot = ggsurv$plot + 
  annotate("text", x = 22, y = .72, label= paste0("Log-rank test:\n", surv_pvalue(sfit2)$pval.txt))
ggsurv
```
![_Kaplan-Meier curves of the BRCA patients' survival data from TCGA grouped by age $<60$ and $\ge 60$. The log-rank test is to compare the two survival distributions corresponding to the two groups of patients._](fig/TCGA_surv_km2.png){width=60%}
...
The following code shows that the nonlinear part of the smoothing spline does not have a significant effect ($p = 0.00013$).
...
- Univariate p-value-based filtering
- Univariate variance-based filtering
...
For demonstration, based on the $100$ mRNA-Seq features from TCGA breast cancer patients preprocessed previously, the code below filters omics features at the statistical significance level $0.2$, i.e. univariate analysis $p < 0.2$.
...
The ideal evaluation of a prognostic model is based on completely independent validation data. 
...
To evaluate a prognostic model, there exist several performance metrics.
These can be *time-dependent* or *time-independent*, with the time-dependent metrics being more informative in general compared to integrated measures (i.e. evaluated across many time points).
...
For one split of train/validation data sets, one can estimate a survival discrimation or calibration metric based on either the training or validation data set, such as AUC, C-index and Brier score and their corresponding time-dependent metrics, see the table below.
Since high-dimensional survival models built on the training data can be overfitted, the conclusion should be drawn based on the evaluation from the validation data.

```{r table, echo=FALSE, message=FALSE, warnings=FALSE, results='asis'}
tabl = "
                      Training data                   Validation data       
            |------------------------------:|------------------------------:|
            | Time-dependent  | Integrated  | Time-dependent  | Integrated  |
            |-----------------|:-----------:|-----------------|:-----------:|
AUC         |       \u2713         |      \u2713      |       \u2713         |      \u2713      |
C-index     |       \u2713         |      \u2713      |       \u2713         |      \u2713      |
Brier score |       \u2713         |      \u2713      |       \u2713         |      \u2713      |
...
cal_train = calibrate(f_train, u = 5, cmethod = "KM", m = nrow(data_train) / 5, B = 200)
cal_validate = calibrate(f_validate, u = 5, cmethod = "KM", m = nrow(data_validate) / 5, B = 200)
